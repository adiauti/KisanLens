...
import { diagnose, type DiagnosisInput, type DiagnosisResult } from '@/data/diagnosis';
import type {
  CropId,
  PlantPart,
  SymptomType,
...
type Step = 'crop' | 'parts' | 'symptoms' | 'conditions' | 'results';

const STEPS: Step[] = ['crop', 'parts', 'symptoms', 'conditions', 'results'];
...
export default function WizardPage() {
  ...
  const goNext = () => {
    ...
    else if (step === 'conditions') setStep('results');
  };
  ...
  const goPrev = () => {
    ...
    else if (step === 'results') setStep('conditions');
  };
  ...
  const toggleConditions = (c: Condition) =>
    setConditions((arr) => (arr.includes(c) ? arr.filter((x) => x !== c) : [...arr, c]));
  ...
  return (
    <div className="container-narrow py-8 sm:py-12">
      ...
      <div className="card-static p-6 sm:p-8 mb-6 min-h-[320px]">
        ...
        {step === 'parts' && (
          ...
          >
            <div className="grid gap-3 grid-cols-2 sm:grid-cols-3">
              {(['leaves', 'stem', 'root', 'fruit', 'flower', 'whole'] as PlantPart[]).map(
                (p) => (
                  <button
                    key={p}
                    type="button"
        ...
        {step === 'results' && <Results results={results} />}
      </div>
...
function Results({ results }: { results: DiagnosisResult[] }) {
  const t = useTranslations('wizard');
  ...
  return (
    ...
    >
      <div
        className={cn(
          'p-5',
          featured && 'border-b border-leaf-100'
        )}
      >
